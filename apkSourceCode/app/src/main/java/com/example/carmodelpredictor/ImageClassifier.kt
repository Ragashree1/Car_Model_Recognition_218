...
import com.example.carmodelpredictor.MainActivity
...
class ImageClassifier(context: Context) {
    ...
    init {
        try {
            model = LiteModuleLoader.load(MainActivity.assetFilePath(context, "car_model_1.pt"))
                //Module.load(assetFilePath(context, "car_model.pt"))
    ...
    fun classifyImage(bitmap: Bitmap): Serializable? {
        ...
        println("First few input tensor values: ${inputTensor.dataAsFloatArray.take(10)}")
...
// Interpret results
        ...
        if (probabilities != null) {
            ...
            println("Error: Output tensor is null")
        }
// Find the index of the maximum probability
        return buildString { "failed to get label" }
    }
